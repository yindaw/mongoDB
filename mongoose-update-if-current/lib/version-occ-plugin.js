...
function versionOCCPlugin(schema) {
  ...
  (0, _assert["default"])(versionKey, 'document schema must have a version key'); // Add pre-save hook to check version

  schema.pre('save', function (next) {
    // Condition the save on the versions matching
    this.$where = _objectSpread({}, this.$where, _defineProperty({}, versionKey, this[versionKey])); // Increment the version atomically
    if (this.increment) {
